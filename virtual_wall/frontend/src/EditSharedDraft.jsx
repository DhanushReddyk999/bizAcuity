...
export default function EditSharedDraft() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      navigate("/Login", { state: { redirect: `/edit-shared/${editId}` } });
      return;
    }
  ...
  useEffect(() => {
    ...
    async function fetchDraft() {
      try {
        ...
        const res = await fetch(`http://localhost:8080/editSharedDraft/${editId}`, {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div className="appHeader">
        Edit Shared Draft: {draftName}
        <span>Edit this altar and save changes</span>
      </div>
